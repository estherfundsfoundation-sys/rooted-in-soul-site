...
export default function StudioClient({ signedIn }: { signedIn: boolean }) {
  ...
  function updateService(index: number, field: "title" | "category" | "description" | "price" | "imageUrl", value: string) {
  ...
  async function uploadServicePhoto(index: number, file?: File) {
    if (!file) return;
    setBusy(true); setMessage("Uploading service photo…");
    try {
      const uploaded = await upload(`rooted-services/${Date.now()}-${file.name}`, file, { access: "public", handleUploadUrl: "/api/gallery/upload", multipart: true });
      updateService(index, "imageUrl", uploaded.url);
      setMessage("Service photo added. Publish website updates when you are ready.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "The service photo did not upload."); }
    finally { setBusy(false); }
  }

  if (!ready) return (
    <form className="studio-card" onSubmit={signIn}>
      <div className="studio-login-gallery" aria-label="Rooted In Soul client work">
        <div className="studio-login-photo"><Image src="/images/client-boho-braids-side.jpeg" alt="Boho braids styled by Rooted In Soul" fill sizes="(max-width: 760px) 50vw, 340px" priority /></div>
        <div className="studio-login-photo"><Image src="/images/client-boho-braids-front.jpeg" alt="Front view of boho braids styled by Rooted In Soul" fill sizes="(max-width: 760px) 50vw, 340px" priority /></div>
      </div>
  ...
  return (
    <div className="studio-dashboard">
      ...
      {contentLoaded && <form className="studio-card studio-panel studio-content-panel" onSubmit={saveContent}>
        ...
        <div className="studio-subhead"><span>Services, photos &amp; pricing</span><button type="button" onClick={() => setContent({ ...content, services: [...content.services, { title: "New service", category: "", description: "", price: "", imageUrl: "" }] })}>+ Add service</button></div>
        <div className="studio-services">{content.services.map((service, index) => <div className="studio-service" key={`${index}-${service.title}`}>
          <div className="studio-service-photo">{service.imageUrl ? <img src={service.imageUrl} alt="" /> : <span>No photo yet</span>}</div>
          <label className="studio-service-upload">Choose service photo<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => uploadServicePhoto(index, event.target.files?.[0])} /></label>
          <input aria-label={`Service ${index + 1} category`} value={service.category ?? ""} onChange={(event) => updateService(index, "category", event.target.value)} placeholder="Category — e.g. Braids" />
          <input aria-label={`Service ${index + 1} name`} value={service.title} onChange={(event) => updateService(index, "title", event.target.value)} placeholder="Service name" />
          <input aria-label={`Service ${index + 1} price`} value={service.price} onChange={(event) => updateService(index, "price", event.target.value)} placeholder="Price or starting price" />
          <textarea aria-label={`Service ${index + 1} description`} value={service.description} onChange={(event) => updateService(index, "description", event.target.value)} placeholder="Short service description" />
          <button type="button" className="studio-remove" onClick={() => setContent({ ...content, services: content.services.filter((_, serviceIndex) => serviceIndex !== index) })}>Remove</button>
        </div>)}</div>
